...
IMGS_PATH = './dummy_data/cropped'
OUT_PATH = './results/extreme_crop/filtering'
...
def belt_detector(net, img, belt_detected):
    ...
    for out in outs:
        for detection in out:
            ...
            if confidence > 0.2:
                ...
                if class_id == 1:
                    belt_detected.add_corner_belt(img)
                elif class_id == 0:
                    belt_detected.add_belt(img)
...
def main():
    ...
    for idx, img_path in enumerate(glob.glob(IMGS_PATH + '/*.jpg')):

        img = cv2.imread(img_path)  # # test_cropped.png  
        ...
        belt_detected = belt_detector(net, img, belt_detected)
        ...
        with open("obj.names","r")as f:
                ...
                cv2.fastNlMeansDenoising(img,img,3,5,11)
